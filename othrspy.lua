...
local oth = typeof(getgenv) == "function" and getgenv().oth or oth or (syn and syn.oth)
if not oth or not oth.hook or not oth.unhook then
    warn("oth 라이브러리를 찾을 수 없습니다.")
    return
end
...
local function disablehooks()
    if hook and unhook then
        ...
        if restorefunction then
            ...
            if originalFunction then pcall(restorefunction, originalFunction) end
        end
    else
        ...
        if originalEvent then hookfunction(Instance.new("RemoteEvent").FireServer, originalEvent) end
        if originalFunction then hookfunction(Instance.new("RemoteFunction").InvokeServer, originalFunction) end
        if originalUnreliableEvent then hookfunction(Instance.new("UnreliableRemoteEvent").FireServer, originalUnreliableEvent) end
    end
end
